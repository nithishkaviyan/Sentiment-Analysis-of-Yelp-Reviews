...
from Yelp_CNN_model import CNN_model
...
f=5
nc=10
in_size=int((seq_length+2*p-f+1)*nc/s)

model=CNN_model(vocab_size,500,in_size,nc,s,p,f)
model.cuda()
...
for epoch in range(num_epochs):
    ...
    for i in range(0,len(x_train),batch_size):
        if ((i+batch_size)>=len(x_train)):
            ...
            Y_tr=list(np.asarray(y_train)[perm[i:i+batch_size]])
    ...
    print("Epoch: "+str(epoch)+" Train Loss: %.4f" %(np.mean(epoch_loss))+\
          " Train Accuracy: %.4f" % (epoch_acc*100.0)+\
          " Epoch Time: "+str(time.time()-time1))
    ...
    for i in range(0,len(y_test),batch_size):
      ...
      optimizer.zero_grad()
      with torch.no_grad():
        tes_loss,tes_score=model(data,target)
